...
type GalaxyId = 'galaxy.01' | 'galaxy.02' | 'galaxy.03' | 'galaxy.04' | 'galaxy.05';
...
export default function Dashboard() {
  ...
  const openPostConfirm = (opp: Opportunity) => {
    ...
    // Pre-fill with the article's og:image (most engaging photo per the
    // publisher's editor). User can clear or override it before confirming.
    setPostingImageUrl(opp.imageUrl ?? '');
  };

  const closePostConfirm = () => {
    if (postingInFlight) return; // don't dismiss mid-post
  ...
  const confirmPostNow = async () => {
    ...
    try {
      const res = await fetch('/api/post-to-x', {
        ...
        body: JSON.stringify({
          text,
          imageUrl: postingImageUrl.trim() || undefined,
        ...
        <div className="px-7 py-6 space-y-5">
          ...
          <div>
            <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider block mb-2">
              Image URL  <span className="text-gray-600 normal-case font-normal">(optional, public http(s) image)</span>
            </label>
            <input
              type="url"
              value={imageUrl}
            ...
            <p className="text-xs text-gray-600 mt-1.5">
              {imageUrl
                ? <>Pre-filled from the source article's social-preview photo. Clear or replace if you prefer a different image.</>
                : <>Max 5MB. Leave empty to post text only. Suggested search: <span className="text-gray-500">{opp.imageSearchQuery || '—'}</span></>}
            </p>
          </div>
